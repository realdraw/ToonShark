...
<p align="center">
  웹툰 PDF·이미지·PSD 파일을 슬라이스하고, 각 플랫폼 규격에 맞춰 내보내는 데스크톱 도구
</p>
...
ToonShark은 웹툰 원본 파일(PDF, JPG/PNG, PSD)을 개별 이미지로 분할하고, 각 연재 플랫폼이 요구하는 너비·포맷·파일 크기에 맞춰 자동으로 내보내주는 도구입니다.

### 작업 흐름

1. **원본 파일 열기** — 드래그 앤 드롭 또는 파일 선택 (PDF / JPG / PNG / PSD)
...
- **다중 포맷 입력** — PDF, JPG, PNG, PSD(Photoshop) 파일 지원
- **여러 PSD 병합** — PSD 2개 이상을 동시에 드롭하면 세로로 이어붙여 하나의 원본으로 슬라이스 (워커 스레드 파싱 + 디스크 스트리밍 파이프라인으로 4억 픽셀급 대용량도 OOM 없이 처리)
- **자동 슬라이스** — 패널 사이 흰색 여백을 분석해 자르기 경계를 자동 탐지
- **고정 슬라이스** — 설정한 픽셀 높이로 균일하게 분할
- **멀티 파일 워크스페이스** — 여러 원본 파일을 탭으로 열어 동시 작업
...
[Releases](https://github.com/realdraw/ToonShark/releases) 페이지에서 설치 파일을 다운로드할 수 있습니다.
